...
describe("summary prompt builders", function()
  local prompts
  before_each(function()
    package.loaded["aicommits.prompts"] = nil
    prompts = require("aicommits.prompts")
  ...
  it("build_chunk_summary_prompt returns system/user with file path and chunk", function()
    local result = prompts.build_chunk_summary_prompt("src/mod.lua", "- foo()\n+ bar()")
  ...
  it("build_file_rollup_prompt returns system/user paragraph prompt", function()
    local result = prompts.build_file_rollup_prompt("a.lua", "- added x\n- removed y")
    assert.is_table(result)
  ...
  it("build_small_batch_prompt returns system/user with batch payload", function()
    local payload = "file1.lua\n---\n+x\n---\nfile2.lua\n---\n+y"
    local result = prompts.build_small_batch_prompt(payload)
    assert.is_table(result)
    assert.is_truthy(result.system:match("bullet per file"))
    assert.is_truthy(result.user:match("file1.lua"))
  end)

  it("no longer exposes build_summary_prompt dispatcher", function()
    assert.is_nil(prompts.build_summary_prompt)
  end)
end)
